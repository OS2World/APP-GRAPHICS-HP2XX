...
\xdef\manvers{$Revision: 1.0.2 $}  % For use in headers, footers too
@end tex
...
This is Edition 1.0.2, April 1993,
...
@sp 1
@c !!set edition, date, version
@subtitle Edition 1.0.2, for HP2XX version 3.1.x
@subtitle April 1993
@author by Heinz W. Werntges
@page
@tex
{\parskip=0pt
\hfill heinz\@convex.rz.uni-duesseldorf.de\par
...
@c !!set edition, date, version
This is Edition 1.0.2, April 1993, for HP2XX Version 3.1.x
@end ifinfo
...
hard margins margins which cannot be avoided. Currently, these modes are
...
(typically @var{black}), and their tip thicknessess are one unit
...
all others are set to vaious sizes.
...
option @samp{-S 4} for CMYK color mode (for the DJ550C). Be aware that
currently there is no data compression built-in, so that hi-res PCL color
output may amount to quite large data volumes.
...
@example
hp2xx -m pcl -d 150 -r90 -h270 -w160 landscpe.hp
@end example
...
use @samp{-m eps}, edit the resulting Encapulated PostScript file, look
...
In addition, plotting all those pels is not really effficiently implemented,
...
(edge rectangle relative) uses the current line type, its couterpart
...
@code{PU   |+  | Pen up}
@code{-----|---|--------------------------------------------------------}
@code{RA   | - | Fill reactangle absolute}
@code{RO   | - | Rotate coordinate system  [use option -r instead!]}
@code{RP   | - | Replot}
@code{RR   | - | Fill reactangle relative}
@code{-----|---|--------------------------------------------------------}
